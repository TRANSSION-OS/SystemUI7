...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private static final int MSG_SIMLOCK_UPDATE = 333;
    ...
    private KeyguardUpdateMonitorCallback mQuickStatusBarHeaderCallback;
    ...
    // talpa@andy 2017/3/20 22:20 add @{
    public KeyguardUpdateMonitorCallback getmQuickStatusBarHeaderCallback() {
        return mQuickStatusBarHeaderCallback;
    }

    public void setmQuickStatusBarHeaderCallback(KeyguardUpdateMonitorCallback mQuickStatusBarHeaderCallback) {
        this.mQuickStatusBarHeaderCallback = mQuickStatusBarHeaderCallback;
    }
    // @}
    @Override
    ...
    private void handleAirplaneModeChanged(boolean airplaneMode) {
        for (int j = 0; j < mCallbacks.size(); j++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(j).get();
            if (cb != null) {
                // talpa@andy 2017/3/20 22:19 modify @{
                if (cb == mQuickStatusBarHeaderCallback) {
                    cb.onAirplaneModeChanged(airplaneMode);
                } else {
                    cb.onRefreshCarrierInfo();
                }
                // @}
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // talpa@andy 2017/3/21 13:56 add:打印intent参数log @{
  /*          String  extars = "";
            for (int i=0; i<intent.getExtras().keySet().size();i++) {
                 String str = intent.getExtras().keySet().iterator().next();
                 if (i==intent.getExtras().keySet().size()-1) {
                     extars += str;
                 } else {
                     extars += str + "|";
                 }
            }
            LogUtil.i(action+">key="+extars);*/
            if (Intent.ACTION_TIME_TICK.equals(action)
                ...
                mHandler.obtainMessage(MSG_SIM_STATE_CHANGE, args.subId, args.slotId, args.simState)
                        .sendToTarget();
                if (KeyguardSupportSimlock.getInstance().isSimlockStatusChange(intent)) {
                ...
                boolean state = intent.getBooleanExtra(TelephonyManager.EXTRA_STATE, false);
    ...
    /**
     * Handle {@link #MSG_SIMLOCK_UPDATE}
     */
...
public boolean getMaxBiometricUnlockAttemptsReached() {
    if (FAILED_BIOMETRIC_UNLOCK_ATTEMPTS_BEFORE_BACKUP <= getFailedUnlockAttempts(sCurrentUser))
        return true;
    else return false;
    }
