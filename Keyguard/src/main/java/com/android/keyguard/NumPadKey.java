...
public class NumPadKey extends ViewGroup {
    ...
    private View.OnClickListener mListener = new View.OnClickListener() {
        ...
        public void onClick(final View thisView) {
            //linwujia add begin
            /*thisView.animate().cancel();
            thisView.animate().scaleX(1.2f).scaleY(1.2f).setDuration(100).withEndAction(new Runnable() {

                @Override
                public void run() {
                    thisView.animate().scaleX(1.0f).scaleY(1.0f).setDuration(50).start();
                }
            }).start();*/
            //linwujia add end
    ...
    protected NumPadKey(Context context, AttributeSet attrs, int defStyle, int contentResource) {
        ...
        try {
            ...
            mUseBackground = a.getBoolean(R.styleable.NumPadKey_usebackground, false);
        } finally {
            a.recycle();
        }
        ...
        if (!a.hasValueOrEmpty(android.R.styleable.View_background)) {
            if(mUseBackground) {
                setBackground(mContext.getDrawable(R.drawable.itel_keyguard_pin_key_selector_bg));
            } else {
