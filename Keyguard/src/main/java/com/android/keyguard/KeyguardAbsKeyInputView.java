    ...
    private void onPasswordChecked(int userId, boolean matched, int timeoutMs,
        ...
        if (matched) {
            ...
            if (timeoutMs == 0) {
                ...
                final int failedAttempts = monitor.getFailedUnlockAttempts(userId) % 5;
                int messageId = 0;
                int remainingFailAttempts = 0;
                switch (failedAttempts) {
                    case 3:
                    case 4:
                        messageId = R.string.kg_password_failed_attempts_message;
                        remainingFailAttempts = 5 - failedAttempts;
                        break;

                    default:
                        messageId = getWrongPasswordStringId();
                        break;
                }
                String message = mContext.getString(messageId, remainingFailAttempts);
                if(getResources().getConfiguration().locale.getLanguage().contains(Locale.ENGLISH.getLanguage())&& remainingFailAttempts == 2)
                    message += "s";
                mSecurityMessageDisplay.setMessage(message, true);
                //zhw add end
                //zhw mSecurityMessageDisplay.setMessage(getWrongPasswordStringId(), true);
            }
        }
        ...
        new CountDownTimer(elapsedRealtimeDeadline - elapsedRealtime, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                mSecurityMessageDisplay.setMessage(
                        ...
                        */
                /* zhw add*/
//                mSecurityMessageDisplay.setMessage(R.string.kg_too_many_failed_attempts_countdown, true,
//                        secondsRemaining);
                /* zhw add end */
    ...
    //linwujia add for adjust LockedView layout when the mode is pin
