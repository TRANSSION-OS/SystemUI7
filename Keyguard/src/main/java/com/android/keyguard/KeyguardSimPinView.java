        ...
        if (SubscriptionManager.isValidSubscriptionId(mSubId)) {
            ...
            if (count < 2) {
                ...
                mRemainTimes = Integer.valueOf(TelephonyManager
                        .getTelephonyProperty(info.getSimSlotIndex(), "gsm.sim.pin.remaintimes", "0"));
                 /* talpa zhw add*/
//                if(mRemainTimes <= 2) {
//                    msg = getPinPasswordErrorMessage(mRemainTimes);
//                } else{
//                    msg = rez.getString(R.string.kg_sim_pin_instructions_multi,
//                            displayName, mRemainTimes);
//                }
                /* talpa zhw add end */
                msg = rez.getString(R.string.kg_sim_pin_instructions_multi, displayName, mRemainTimes);//add by wangying 
                Log.i("zhw msg", "msg :+ " + msg);
                /* @} */
                if (info != null) {
    ...
    private String getPinPasswordErrorMessage(int attemptsRemaining) {
        ...
        if (attemptsRemaining == 0) {
            ...
            displayMessage = getContext().getResources().getQuantityString(
                    R.plurals.kg_password_wrong_pin_code, attemptsRemaining,
                    attemptsRemaining);
        } else {
            displayMessage = getContext().getString(
    ...
    private abstract class CheckSimPin extends Thread {
        ...
        public void run() {
            try {
                if (DEBUG) {
                    Log.v(TAG, "call supplyPinReportResultForSubscriber(subid="
                            + mSubId + ")");
                }
    ...
    protected void verifyPasswordAndUnlock() {
        ...
        //SPRD: modify for bug 587763
        if (entry.length() < 4 || entry.length() > 8) {
            // otherwise, display a message to the user, and don't submit.
            mSecurityMessageDisplay.setMessage(
                    R.string.kg_invalid_sim_pin_hint, true);
                        ...
                        public void run() {
                            ...
                            if (result == PhoneConstants.PIN_RESULT_SUCCESS) {
                                ...
                                if (result == PhoneConstants.PIN_PASSWORD_INCORRECT) {
                                    ...
                                    // "PIN operation failed!" - no idea what
                                    // this was and no way to
                                    // find out. :/
                                    mSecurityMessageDisplay
                                            .setMessage(
                                                    getContext()
