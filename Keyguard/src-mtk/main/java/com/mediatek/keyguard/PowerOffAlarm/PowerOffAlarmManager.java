...
public class PowerOffAlarmManager {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case RESHOW_KEYGUARD_LOCK:
                    ...
                    // Talpa PeterHuang Add for Delay NORMAL_BOOT_DONE_ACTION 4000 To 6500 Fixed bug tfs#7974 @{
                    postDelayed(new Runnable() {
                        @Override
                        public void run() {
    ...
    public void startAlarm() {
        ...
        // Talpa:PeterHuang add For Fixed Bug,here comming twice..  translate from S31@{
        if ((SystemClock.elapsedRealtime() - mLastStartTime) < 5000) {
            Log.d(TAG, "duplicate ignore.." );
            return;
        }
        mLastStartTime = SystemClock.elapsedRealtime();
        // @}

        // Talpa:PeterHuang Add for Delay startAlarm Fixed bug tfs#6989 translate from S31 @{
        mHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                Log.d(TAG, "systemui keyguardViewMediator startAlarm..");
                startAlarmPrivate();
            }
        }, 2000);
        // @}
