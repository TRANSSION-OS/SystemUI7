...
//    }
    ...
    public static boolean isWifiOnlyDevice(Context sContext) {
    ...
    public static boolean isWifiDisplaySupport(Context sContext) {
     ...
     */
    public static boolean isRadioOn(int subId, Context sContext) {
    ...
    //itel@andy add @{
    public static boolean isSimEnable(List<SubscriptionInfo> infos, int slotId, Context context) {
        return isSimInsertedBySlot(infos, slotId)
                && isRadioOnBySlotId(slotId, context)
                && !isSimLocked(slotId);
    }

    public static boolean isRadioOnBySlotId(int slotId, Context context) {
        int subId1 = SIMHelper.getFirstSubInSlot(slotId);
        return SIMHelper.isRadioOn(subId1, context);
    }

    public static boolean isSimLocked(int slotId) {
    ...
    public static boolean isSimInsertedBySlot(List<SubscriptionInfo> infos,int slotId) {
     ...
     * @return
     */
    public static boolean isMobileSimEnable(Context context) {
        List<SubscriptionInfo> infos =
                SubscriptionManager.from(context).getActiveSubscriptionInfoList();
    	if (infos == null || infos.size() == 0) {
    		return false;
    	} 
    	for (int i = 0; i < SIMHelper.getSlotCount(); i ++) {
            if (isSimEnable(infos, i, context)) {
                return true;
            }
        }
        return false;
     ...
     * @return
     */
    public static boolean isMobileSimInserted(Context context) {
        List<SubscriptionInfo> infos =
                SubscriptionManager.from(context).getActiveSubscriptionInfoList();
        if (infos == null || infos.size() == 0) {
            return false;
