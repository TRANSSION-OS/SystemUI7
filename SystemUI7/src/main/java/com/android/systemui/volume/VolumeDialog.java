...
public class VolumeDialog implements TunerService.Tunable {
    ...
    private void updateVolumeRowH(VolumeRow row) {
        ...
        // update icon
        /// George:changed for mtk;
        final boolean iconEnabled;
        boolean isSprid = TalpaUtils.isSPRDPlatform();
        if(isSprid) {
             iconEnabled = (mAutomute || ss.muteSupported) && !zenMuted && !isZenNone;
        }else {
            iconEnabled = (mAutomute || ss.muteSupported) && !zenMuted ;
        }
        /// end
        ...
        if (iconRes != row.cachedIconRes) {
            ...
            row.icon.setImageResource(iconRes);
        }else if(!isZenNone&&isSprid/* /// George:change for mtk */){
            row.cachedIconRes = iconRes;
            row.icon.setImageResource(iconRes);
        }
        row.iconState =
        ...
        if(isSprid) {
             enableSlider = !zenMuted && !isZenNone;
        }else {
    ...
    private final VolumeDialogController.Callbacks mControllerCallbackH
        ...
        public void onLayoutDirectionChanged(int layoutDirection) {
            //mDialogView.setLayoutDirection(layoutDirection); // remove by lych for fix bug cdm#22887
        }
