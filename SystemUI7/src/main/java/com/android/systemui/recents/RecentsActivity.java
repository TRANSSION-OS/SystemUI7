...
import com.android.systemui.recents.util.RecentUtil;
import com.android.systemui.recents.views.CircleProgressTalpaBar;
...
public class RecentsActivity extends Activity implements ViewTreeObserver.OnPreDrawListener {
    ...
    // TALPA DepngHuang add start
    private TextView mMemoryShowTV;
    //TALPA bo.yang1 modify for CircleProgressTalpaBar
    private CircleProgressTalpaBar mClearButton;
    //TALPA bo.yang1 modify end
    private long mOldAvailMemory;
    //TALPA bo.yang1 add for AvailMemory before click
    private long mPreClickAvailMemory;
    //TALPA bo.yang1 add end
    private Context mContext;
    // TALPA DepngHuang add end
	...
	// TALPA bo.yang1 add 
	private Task mLaunchTask=null;
    private boolean isAllClearTask=false;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_SCREEN_OFF)) {
                ...
                Prefs.putLong(RecentsActivity.this, Prefs.Key.OVERVIEW_LAST_STACK_TASK_ACTIVE_TIME,
                        0);
                // Talpa bo.yang for evictAll mActivityLabelCache to head title change
                // when ACTION_LOCALE_CHANGED @{
            }else if (action.equals(Intent.ACTION_LOCALE_CHANGED)) {
                RecentsTaskLoader loader = Recents.getTaskLoader();
                loader.evictAllActivityLabelCache();
            }
                //@}
    ...
    boolean dismissRecentsToLaunchTargetTaskOrHome() {
        ...
        if (ssp.isRecentsActivityVisible()) {
			...
			// TALPA bo.yang1 add @{
			if(mLaunchTask !=null && mLaunchTask.isLaunchTarget && isAllClearTask){
                ActivityOptions opts = ActivityOptions.makeBasic();
                if (ssp.startActivityFromRecents(mContext, mLaunchTask.key, mLaunchTask.title, opts)) {
                  return  true;
                }
            }
			//@}
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        takeKeyEvents(true);
        mRecentsView = (RecentsTalpaView) findViewById(R.id.recents_view);
        ...
        // Talpa bo.yang1 add ACTION_LOCALE_CHANGED for head title @{
        filter.addAction(Intent.ACTION_LOCALE_CHANGED);
        //@}
        registerReceiver(mSystemBroadcastReceiver, filter);

        /* SPRD: Bug 475644 new feature of quick cleaning. @{ */
        boolean isSupportClearAllTasks = RecentsActivityUtils.getInstance().isSupportClearAllTasks();
        if(isSupportClearAllTasks){
            // Talpa bo.yang1 modify for CircleProgressTalpaBar and init Progress @{
            mClearButton = (CircleProgressTalpaBar) findViewById(R.id.clear_button);
            mClearButton.setProgress((int)RecentUtil.getRamAvailableSize(this),false);
            // @}
            mClearButton.setVisibility(View.VISIBLE);
            RecentsActivityUtils.getInstance().init(this,mRecentsView, mClearButton);
            mClearButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {

                    // Talpa:PeterHuang fixed cdn#9996 @{
                    if(mClearButton.getAlpha() < 0.1f) {
                        LogUtil.d("clear button status:" + mClearButton.getAlpha());
                        return ;
                    }
                    //@}
                    //TALPA bo.yang1 add for AvailMemory before click @{
                    mPreClickAvailMemory=mOldAvailMemory;
                    //@}
                    EventBus.getDefault().send(new DismissAllTaskViewsEvent());
                    // Talpa bo.yang1 add set Progress @{
                    mClearButton.setProgress((int)RecentUtil.getRamAvailableSize(RecentsActivity.this),true);
                    //@}
                    ...
                    mHandler.postDelayed(new Runnable(){
                        @Override
                        public void run() {
                            long newAvailMemory = RecentUtil.getRamAvailableSize(mContext);
                            //TALPA bo.yang1 modify for AvailMemory  @{
                            long clearMemory = mOldAvailMemory - mPreClickAvailMemory;
                            if(clearMemory<=0) {
                                clearMemory = newAvailMemory - mPreClickAvailMemory;
                            }
                            //@}

                            //TALPA bo.yang1 add virtual for AvailMemory  @{
                            /*if (clearMemory > 0) {
                                String message = mContext.getString(R.string.recents_clear_memory, clearMemory);
        ...
        // Talpa DepingHuang add @{
        mContext = this;
        updateMultiWindowModeView(this.isInMultiWindowMode());
        //@}
    }
    ...
    private void  updateRamInfo(){
        if (mMemoryShowTV == null)
            mMemoryShowTV = (TextView) findViewById(R.id.memory_show);
        mOldAvailMemory = RecentUtil.getRamAvailableSize(this);
        String memoryStr = getResources().getString(R.string.mem_info,
                /*RecentUtil.getRamAvailableSizeFormat(mOldAvailMemory)*/
                RecentUtil.getRamAvailableSizeFormat(this),
                RecentUtil.getRamTotalSizeFormat(this));
        mMemoryShowTV.setText(memoryStr);
    }

    private void updateMultiWindowModeView(boolean isInMultiWindowMode){
        if (isInMultiWindowMode) {
            mMemoryShowTV.setVisibility(View.GONE);
            mClearButton.setVisibility(View.GONE);
        }
        else {
            mMemoryShowTV.setVisibility(View.VISIBLE);
            if (null != mClearButton){
    ...
    private void reloadStackView() {
        ...
        if (!loadPlan.hasTasks()) {
            // Talpa:PeterHuang modify for mulitwindow not show excludedTask @{
        ...
        // Talpa:bo.yang1 add For getAlpha()@{
        if(mClearButton!=null && mClearButton.getAlpha() != 1.0f) {
            mClearButton.setAlpha(1.0f);
        }
        if(mMemoryShowTV!=null && mMemoryShowTV.getAlpha() != 1.0f) {
		...
		// Talpa:bo.yang1 add@{
		mLaunchTask=null;
        isAllClearTask=false;
		//@}

        // Talpa:PeterHuang add For not task to invisible clear btn@{
        if (null != mClearButton){
            if (mStack.getStackTaskCount() != 0){
                mClearButton.setVisibility(View.VISIBLE);
            }
    ...
    public final void onBusEvent(final MultiWindowStateChangedEvent event) {
        LogUtil.i("event.inMultiWindow: " + event.inMultiWindow);
        /**
         * 重新onCreate RecentsActivity时有判断当前为全屏模式但实际为分屏的情况（会有一个时间差的延后）
         * 这里做容错处理
         */
        updateMultiWindowModeView(event.inMultiWindow);
    }
    ...
    public final void onBusEvent(DeleteTaskDataEvent event) {
        ...
        SystemServicesProxy ssp = Recents.getSystemServices();
		// Talpa:bo.yang1 modify for LaunchTarget @{
        if(!event.task.isLocked  && !(event.task.isLaunchTarget && isAllClearTask)){
            // TALPA:  PeterHuang modified  有可能为从未加载的task，这时task isLocked是未被赋值的。
            // 当用户直接按了 clean all task会被误清理..@{
            SharedPreferences sharedPreference = PreferenceManager
                    .getDefaultSharedPreferences(mContext.getApplicationContext());
            if (!sharedPreference.contains(event.task.key.toStringKey())) {
                ssp.removeTask(event.task.key.id);
            }
            // @}
        }
		
		// Talpa:bo.yang1 add for save LaunchTarget @{
		if(event.task.isLaunchTarget){
            mLaunchTask=event.task;
        }
		//@}
    ...
    public final void onBusEvent(AllTaskViewsDismissedEvent event) {
		...
		// Talpa:bo.yang1 modify for LaunchTarget @{
		RecentsActivityLaunchState launchState = Recents.getConfiguration().getLaunchState();
        if(event.isAllClearTask()) {
            //一键清理进入
            isAllClearTask=true;
            // bo.yang1 modify for not delete AllTask @{
        }else {
            if (launchState.launchedFromHome) {
                dismissRecentsToHome(false /* animateTaskViews */);
            } else {
