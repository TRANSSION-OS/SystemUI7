...
public final class KeyboardShortcuts {
    ...
    private KeyboardShortcutGroup getDefaultApplicationShortcuts() {
        ...
        /* SPRD 599538 Get Messaging label from PM @{ */
        final String messagingLabel = getPackageLabel(Intent.CATEGORY_APP_MESSAGING, userId);
        /* @} */
        if (messagingIcon != null) {
            keyboardShortcutInfoAppItems.add(new KeyboardShortcutInfo(
                    /* SPRD 599538 Get Messaging label from PM @{ */
                    messagingLabel != null ? messagingLabel :
                        mContext.getString(R.string.keyboard_shortcut_group_applications_im),
                    /* @} */
                    messagingIcon,
                    /* SPRD 599539 Correct Keyboard shortcut for Messaging @{ */
                    KeyEvent.KEYCODE_S,
                    /* @} */
                    KeyEvent.META_META_ON));
        }
    ...
    /* SPRD 599538 Get Messaging label from PM @{ */
    private String getPackageLabel(String intentCategory, int userId) {
        final Intent intent = new Intent(Intent.ACTION_MAIN);
        intent.addCategory(intentCategory);
        try {
            ResolveInfo handler;
            ...
            return handler.loadLabel(mContext.getPackageManager()).toString();
        } catch (RemoteException e) {
            Log.e(TAG, "PackageManagerService is dead", e);
            return null;
        }
