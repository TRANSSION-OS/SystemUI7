...
public class NavigationBarInflaterView extends FrameLayout implements
    ...
    private void createInflaters() {
        ...
        mLandscapeInflater = LayoutInflater.from(mContext
                .createConfigurationContext(landscape));
    }
    ...
    private void inflateChildren() {
        removeAllViews();
        mRot0 = (FrameLayout) mLayoutInflater.inflate(
                R.layout.navigation_layout, this, false);
        mRot0.setId(R.id.rot0);
        addView(mRot0);
        mRot90 = (FrameLayout) mLayoutInflater.inflate(
                R.layout.navigation_layout_rot90, this, false);
    ...
    public void setButtonDispatchers(
            SparseArray<ButtonDispatcher> buttonDisatchers) {
    ...
    private void addAll(ButtonDispatcher buttonDispatcher, ViewGroup parent) {
        for (int i = 0; i < parent.getChildCount(); i++) {
            // Need to manually search for each id, just in case each group has
            // more than one
            // of a single id. It probably mostly a waste of time, but shouldn't
            // take long
    ...
    protected void inflateLayout(String newLayout) {
        ...
        /* SPRD: Bug 535100 new feature of dynamic navigationbar @{ */
        // Inflate these in start to end order or accessibility traversal will
        // be messed up.
    ...
    private void addGravitySpacer(LinearLayout layout) {
        layout.addView(new Space(mContext), new LinearLayout.LayoutParams(0, 0,
                1));
    }

    private void inflateButtons(String[] buttons, ViewGroup parent,
            boolean landscape) {
    ...
    private ViewGroup.LayoutParams copy(ViewGroup.LayoutParams layoutParams) {
        if (layoutParams instanceof LinearLayout.LayoutParams) {
            return new LinearLayout.LayoutParams(layoutParams.width,
                    layoutParams.height,
    ...
    @Nullable
    protected View inflateButton(String buttonSpec, ViewGroup parent,
            boolean landscape, int indexInParent) {
        LayoutInflater inflater = landscape ? mLandscapeInflater
                : mLayoutInflater;
    ...
    public static String extractImage(String buttonSpec) {
        ...
        String subStr = buttonSpec.substring(start + 1,
                buttonSpec.indexOf(KEY_CODE_END));
        return subStr;
    }
    ...
    public static int extractKeycode(String buttonSpec) {
        ...
        String subStr = buttonSpec.substring(start + 1,
                buttonSpec.indexOf(KEY_IMAGE_DELIM));
    ...
    public static float extractSize(String buttonSpec) {
        ...
        String sizeStr = buttonSpec.substring(sizeStart + 1,
                buttonSpec.indexOf(SIZE_MOD_END));
     ...
     * overriding the configuration from the overridden resources aren't loaded
     * currently.
     */
    private void setupLandButton(View v) {
        Resources res = mContext.getResources();
        v.getLayoutParams().width = res
                .getDimensionPixelOffset(R.dimen.navigation_key_width_sw600dp_land);
        int padding = res
                .getDimensionPixelOffset(R.dimen.navigation_key_padding_sw600dp_land);
