...
import itel.transsion.settingslib.utils.TalpaResUtil;
...
public class NotificationChildrenContainer extends ViewGroup {
    ...
    // talpa@tj 2017/6/5 modify:优化通知高度调整以后分组通知折叠个数 @{
    private static final int NUMBER_OF_CHILDREN_WHEN_SYSTEM_EXPANDED = 2;
    //@}
...
//                R.dimen.notification_divider_height));
        ...
        mNotificationHeaderMargin = getResources().getDimensionPixelSize(
                TalpaResUtil.android.dimen.notification_content_margin_top());
        //调整分组通知头高度
//        mNotificatonTopPadding = getResources().getDimensionPixelSize(
//                R.dimen.notification_children_container_top_padding);
        mNotificatonTopPadding = mHeaderHeight - mNotificationHeaderMargin ;//mNotificationHeaderMargin为37.5dp,mHeaderHeight为36dp

        mCollapsedBottompadding = getResources().getDimensionPixelSize(
                TalpaResUtil.android.dimen.notification_content_margin_bottom());
    }
    ...
    public void removeNotification(ExpandableNotificationRow row) {
        ...
        if (!row.isRemoved()) {
            ...
            //add tj-restore notification header bg after cancel grouping notifications-start
            if (row.getNotificationHeader() != null) {
                row.getNotificationHeader().setBackground(getResources().getDrawable(R.drawable.itel_notification_header_normal_bg));
            }
            row.setBackground(null);
            //add tj-restore notification header bg after cancel grouping notifications-end
        }
    }
    ...
    public void recreateNotificationHeader(OnClickListener listener, StatusBarNotification notification) {
        ...
        if (mNotificationHeader == null) {
            ...
            final View expandButton = mNotificationHeader.findViewById(
                    TalpaResUtil.android.id.expand_button());
    ...
    public void getState(StackScrollState resultState, StackViewState parentState) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childrenExpanded) {
                ...
                // talpa@tj 2017/6/6 grouping notification change notification header bg & content bg @{
                if (child.getNotificationHeader() != null) {
                    child.getNotificationHeader().setBackground(getResources().getDrawable(R.drawable.itel_notification_header_container_bg));
                }
                if (i != childCount -1){
                    child.setBackground(getResources().getDrawable(R.drawable.itel_notification_content_unnormal_bg));
                }else {
                    child.setBackground(getResources().getDrawable(R.drawable.itel_notification_content_unnormal_down_bg));
                }
                //@}
            } else {
