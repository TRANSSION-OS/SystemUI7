...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    // Talpa:PeterHuang add for super power save @{
    @Override // NotificationData.Environment
    public boolean isSuperPowerSaveMode() {
        return TalpaUtils.isSuperPowerSaveMode(mContext);
    }
    // @}
    ...
    private RemoteViews.OnClickHandler mOnClickHandler = new RemoteViews.OnClickHandler() {
        ...
        private void logActionClick(View view) {
            ...
            if (view.getId() == Resources.getSystem().getIdentifier("action0", "id","android") &&
        ...
        private boolean handleRemoteInput(View view, PendingIntent pendingIntent, Intent fillInIntent) {
            Object tag = view.getTag(Resources.getSystem().getIdentifier("remote_input_tag", "id","android"));
                ...
                if (recentTask != null && recentTask.size() > 0) {
                    ...
                    if (user != null && user.isManagedProfile()) {
                        ...
                        text.setCompoundDrawablesRelativeWithIntrinsicBounds(
                                R.drawable.itel_stat_sys_managed_profile_status, 0, 0, 0);
    ...
    protected void notifyUserAboutHiddenNotifications() {
        if (0 != Settings.Secure.getInt(mContext.getContentResolver(),
            ...
            final int colorRes = Resources.getSystem().getIdentifier("system_notification_accent_color", "color","android");
    ...
    protected void applyColorsAndBackgrounds(StatusBarNotification sbn,
        ...
        if (entry.getContentView().getId()
                != Resources.getSystem().getIdentifier("status_bar_latest_event_content", "id","android")) {
    ...
    public boolean isMediaNotification(NotificationData.Entry entry) {
        ...
        return entry.getExpandedContentView() != null &&
               entry.getExpandedContentView()
                       .findViewById(Resources.getSystem().getIdentifier("media_actions", "id","android")) != null;
    }
    ...
    protected NotificationData.Entry createNotificationViews(StatusBarNotification sbn) {
        ...
        //add-by-tj- temporary screen the black notificaiton of qq -tfs#12999-start
        if (isQQblackNotification(sbn,entry)){
            return null;
        }
        //add-by-tj- temporary screen the black notificaiton of qq -tfs#12999-end
    ...
    /**
     * judge whether is the black qq notification
     * @param sbn
     * @param entry
     * @return
     */
    private boolean isQQblackNotification(StatusBarNotification sbn, NotificationData.Entry entry){
        if (sbn.getPackageName().equals("com.tencent.mobileqq")) {
            try {
                Context c = mContext.createPackageContext("com.tencent.mobileqq", Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
                entry.cacheContentViews(mContext, null);
                if (entry.cachedContentView != null && c != null ) {
                    int indentify = c.getResources().getIdentifier("notification_root", "id", "com.tencent.mobileqq");
                    int layoutId = entry.cachedContentView.getLayoutId();
                    LayoutInflater inflater = (LayoutInflater) c.getSystemService(
                            Context.LAYOUT_INFLATER_SERVICE);
                    View mview = inflater.inflate(layoutId, null);
                    View qqBlackNoti = mview.findViewById(indentify);
                    if (qqBlackNoti != null) {
                        return true;
                    }
                }
    ...
    public StatusBarIconView createIcon(StatusBarNotification sbn) {
        ...
        if (!iconView.set(ic, sbn.getPackageName())) {
    ...
    protected void updateRowStates() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (onKeyguard) {
                ...
                // Talpa:PeterHuang add superPower Mode clean notification@}
                if(this.isSuperPowerSaveMode()) {
            ...
            if (suppressedSummary || (isLockscreenPublicMode() && !mShowLockscreenNotifications) ||
                ...
                if (onKeyguard && showOnKeyguard && !childNotification && !suppressedSummary) {
                    mKeyguardIconOverflowContainer.getIconsView().addNotification(entry, entry.notification.getPackageName());
                }
            } else {
        ...
        //modified begin by lych for fix bug cdn#9988
        //mStackScroller.changeViewPosition(mDismissView, mStackScroller.getChildCount() - 1);
        mStackScroller.changeViewPosition(mEmptyShadeView, mStackScroller.getChildCount() - 1/*2*/);
        mStackScroller.changeViewPosition(mKeyguardIconOverflowContainer,
                mStackScroller.getChildCount() - 2/*3*/);
        //modified end by lych for fix bug cdn#9988
    }
    ...
    public void updateNotification(StatusBarNotification notification, RankingMap ranking) {
        ...
        if (applyInPlace) {
            ...
            try {
                if (entry.icon != null) {
                    ...
                    if (!entry.icon.set(ic, notification.getPackageName())) {
        ...
        if (!updateSuccessful) {
            ...
            final StatusBarIcon ic = new StatusBarIcon(
                    ...
                    StatusBarIconView.contentDescForNotification(mContext, n));
            entry.icon.setNotification(n);
            entry.icon.set(ic, notification.getPackageName());
            inflateViews(entry, mStackScroller);
        }
    ...
    protected void updatePublicContentView(Entry entry,
        ...
        if (entry.autoRedacted && publicContentView != null && inflatedView != null) {
            ...
            String notificationHiddenText = mContext.getString(disabledByPolicy
                    ? Resources.getSystem().getIdentifier("notification_hidden_by_policy_text", "string","android")
                    : Resources.getSystem().getIdentifier("notification_hidden_text", "string","android"));
