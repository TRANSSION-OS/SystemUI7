...
import itel.transsion.systemui.ChargingLeonids.ChargeAnimator;
import itel.transsion.systemui.accidenttouch.AccidentTouchWindowManager;
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    private ChargeAnimator mChargeAnimator;
    private boolean isScreemTurnOn;
    //add by wangying end
    AccidentTouchWindowManager mAccidentWindowManager;
    ...
    //linwujia add begin
    protected LeatherWindowView mLeatherWindow;
    protected LeatherWindowManager mLeatherWindowManager;
    //linwujia add end
    ...
    private ContentObserver mUserSetupObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            final boolean userSetup = 0 != Settings.Secure.getIntForUser(
                    ...
                    0 /*default */,
                    mCurrentUserId);
            /// vincent: set default usb mode Mtp
            if(userSetup){
            	StorageManager mStoragemanager=(StorageManager)mContext.getSystemService(Context.STORAGE_SERVICE);
            	final List<VolumeInfo> storagevolumelist = mStoragemanager.getVolumes();
            	boolean isSdcardExist=false;
            	if(storagevolumelist!=null){
            		for(VolumeInfo volume : storagevolumelist){
            			if(volume.getType()==VolumeInfo.TYPE_PUBLIC && volume.isMountedWritable()){
            				isSdcardExist = true;
            				break;
            			}
            		}
    ...
    //add by tj start
    private TextView chargeStateTv;
    private View keyguardCover;
    private View btPanelBottom;
    private int mChargeFullShowTime = 0;
    //add by tj end
    ...
    //linwujia add begin
    private boolean returnCallShowing = false;
    ...
    public void start() {
        /* SPRD: Bug 535096 new feature of lock recent apps @{ */
        mSupportLockApp = SystemUILockAppUtils.getInstance().isSupportLockApp();
        /* @} */
        /* SPRD: Bug 474763 support transition on low-ram device. @{ */
        try {
            mHighEnd = mContext.getResources()
                .getBoolean(Resources.getSystem().getIdentifier("config_force_use_color_view", "bool","android"));
        ...
        //linwujia add begin
        mBatterySettingObserver = new BatterySettingObserver(mContext);
        //linwujia add end
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        //linwujia add begin
        mStatusBarView.setBatterySettingObserver(mBatterySettingObserver);
        //linwujia add end
        ...
        //linwujia add begin
        telephoneBack = (TelephoneBackView) mStatusBarWindow.findViewById(R.id.telephone_back_layout);
        keyguard_call_back = (TelephoneBackView) mStatusBarWindow.findViewById(R.id.keyguard_call_back);

        telephoneBack.setBatterySettingObserver(mBatterySettingObserver);
        keyguard_call_back.setBatterySettingObserver(mBatterySettingObserver);
        //linwujia add end

        mKeyguardStatusBar = (KeyguardStatusBarView) mStatusBarWindow.findViewById(R.id.keyguard_header);
        //linwujia add begin
        mKeyguardStatusBar.setBatterySettingObserver(mBatterySettingObserver);
        //linwujia add end
        ...
        // add by tj start
        chargeStateTv = (TextView) mKeyguardBottomArea.findViewById(R.id.charge_state);
        keyguardCover = (View) mKeyguardBottomArea.findViewById(R.id.keyguard_cover);
        btPanelBottom = mKeyguardBottomArea.findViewById(R.id.btPanel_bottom);
        //add by tj end

        //mKeyguardBottomArea.setAssistManager(mAssistManager);
		//add by wangying begin
        mChargeAnimator = (ChargeAnimator) mNotificationPanel.findViewById(R.id.charge_anim);
        mChargeAnimator.setStatusBar(this);
		//add by wangying end
        ...
        mChargeAnimator.setBatteryController(mBatteryController);
        mBatteryController.addStateChangedCallback(new BatteryStateChangeCallback() {
            ...
            public void onBatteryLevelChanged(int level, boolean pluggedIn, boolean charging) {
                // noop
                Log.v("onBatteryLevelChanged",
                        "onBatteryLevelChanged0000=" + level + "pluggedIn=" + pluggedIn + "charging=" + charging+":"+mChargeFullShowTime);
                if(level==100 && mChargeFullShowTime > 3){
                    // add by tj start
//					setChargeAnimation(level, pluggedIn, false);
                    chargeStateTv.setText(computePowerIndication(level));
//					setPluggedAnimation();
                    // add by tj end
                }else{
                    if(level==100){
                        ...
                        mChargeFullShowTime = 0;
                    }
                    // add by tj start
//					setChargeAnimation(level, pluggedIn, charging);
                    //change by linwujia start
                    chargeStateTv.setText(computePowerIndication(level));
                    //change by linwujia end
//					setPluggedAnimation();
                    // add by tj end
                }
            }

            //add by linwujia for fix bug
            public String computePowerIndication(int level)  {
                if(level >= 100) {
                    return mContext.getResources().getString(R.string.keyguard_charged);
                } else if(level >= 0 && level < 100) {
                    return mContext.getResources().getString(R.string.keyguard_plugged_in_percent, level);
                }
        ...
        if (container != null) {
            ...
            container.addInflateListener(new InflateListener() {
                ...
                public void onInflated(View v) {
                    ...
                    //add by wangying fix tfs 11847
                    qsContainer.setKeyguardShowing(getBarState() == StatusBarState.KEYGUARD);
        ...
        //linwujia add begin
        telephoneBack.setBatteryController(mBatteryController);
        keyguard_call_back.setBatteryController(mBatteryController);
        //linwujia add end
    ...
    //linwujia add begin
    protected void inflateLeatherWindow(Context context) {
        mLeatherWindow = (LeatherWindowView) View.inflate(context, R.layout.leather, null);
    }
    //linwujia add end
    ...
    public void takeToKeyguard() {
        getComponent(KeyguardViewMediator.class).doKeyguardTimeout(null);
    }
    ...
    public int getStatusBarHeight() {
        if (mNaturalBarHeight < 0) {
            ...
            mNaturalBarHeight =
                    res.getDimensionPixelSize(Resources.getSystem().getIdentifier("status_bar_height", "dimen","android"));
        }
    ...
    protected void toggleSplitScreenMode(int metricsDockAction, int metricsUndockAction) {
        // Talpa DepingHuang add for super power save mode@{
    ...
    public void addNotification(StatusBarNotification notification, RankingMap ranking,
        ...
        //talpa@tj 通知栏下拉状态下不显示横屏 @{
//        if (isHeadsUped) {
        if (isHeadsUped && !mNotificationPanel.isFullyExpanded()) {
            // @}
        ...
        setAreThereNotifications();
        updateNotificationClearAll(); // add by lych
    }
    ...
    public void updateNotificationClearAll(){
        mNotificationPanel.setClearAllViewHide();
    }
    ...
    public void onScreenTurnedOff() {
        isScreemTurnOn = false;
        mFalsingManager.onScreenOff();
        if (null != mLeatherWindowManager) {
            mLeatherWindowManager.onScreenTurnedOff();
        }
        //add by wangying begin
        if(mAccidentWindowManager != null){
            if(mAccidentWindowManager.isOpen())
            mAccidentWindowManager.onScreenTurnedOff();
        }

        showChargeAnimation(getBarState(), false);
        //add by wangying end
    }
    ...
    private void addAccidentTouchWindow(){
        if(mAccidentWindowManager == null){
            mAccidentWindowManager = new AccidentTouchWindowManager(mContext);
        }
        if(mAccidentWindowManager.isOpen())
            mAccidentWindowManager.registerTouchSensor();
    ...
    //linwujia add begin
    private void addLeatherWindow() {
        inflateLeatherWindow(mContext);
        mLeatherWindowManager = new LeatherWindowManager(mContext);
        mLeatherWindowManager.setPhoneStatusBar(this);
        mLeatherWindowManager.add(mLeatherWindow);
    }
...
//                if (StatusBarState.KEYGUARD == getBarState() ) {
//                    setPluggedAnimation();
//                }
//                setPluggedAnimation();//add by wangying
            }// add by tj end
            //add by wangying begin
    ...
    public void showKeyguard() {
        ...
        updateNotificationClearAll();//add by lych 9048
    }
    ...
    public boolean onBackPressed() {
        ...
        if (mNotificationPanel.isQsExpanded()
                || mNotificationPanel.isQsDetailShowing()/*talpa zhw add*/) {
    ...
    public void onActivated(ActivatableNotificationView view) {
        ...
        onHintFinished(); //add by wangying fix cdn 9547
    ...
    public void setBarState(int state) {
        ...
        mState = state;
        showChargeAnimation(state, true);
        mGroupManager.setStatusBarState(state);
        mFalsingManager.setStatusBarState(state);
        mStatusBarWindowManager.setStatusBarState(state);
        updateDozing();
        updateNotificationClearAll();
    }

    @Override
    public void onActivationReset(ActivatableNotificationView view) {
        if (view == mStackScroller.getActivatedChild()) {
//            mKeyguardIndicationController.hideTransientIndication();//add by wangying fix cnd 9547
            mStackScroller.setActivatedChild(null);
        }
    }
    ...
    public boolean onDraggedDown(View startingChild, int dragLengthY) {
        // Talpa:PeterHuang add for super power forbidden drag down@{
        if (isSuperPowerSaveMode()){
            return false;
        }
        // @}
...
//        mFalsingManager.onScreenTurningOn(); add by wangying
    ...
    public void onScreenTurnedOn() {
        ...
        if (null != mLeatherWindowManager) {
            mLeatherWindowManager.onScreenTurnedOn();
        }
        //add by wangying begin
        if(mAccidentWindowManager != null && (mLeatherWindowManager == null
                || !mLeatherWindowManager.isVisibility())){
            if(mAccidentWindowManager.isOpen())
                mAccidentWindowManager.onScreenTurnedOn();
        }

        showChargeAnimation(getBarState(), true);
        //add by wangying end
    }
    ...
    //add by wangying begin
    public void showChargeAnimation(int state, boolean isShow) {
        if (isSuperPowerSaveMode()
                || !isPowerPluggedIn()
                || state != StatusBarState.KEYGUARD || ChargeAnimator.DEBUG) {
            mChargeAnimator.stopParticle();
            mChargeAnimator.getCircleAnimator().stop();
            mChargeAnimator.setVisibility(View.INVISIBLE);
            return;
        }
        mChargeAnimator.setVisibility(View.VISIBLE);
        if (!isScreemTurnOn || !isShow) {
            mChargeAnimator.stopParticle();
            mChargeAnimator.getCircleAnimator().stop();
            return;
        }
        if (!mChargeAnimator.isStartAnim()) {
            mChargeAnimator.setChargeState(ChargeAnimator.CHARGE_PULL_IN);
            return;
        } else if (!mChargeAnimator.getFrameAnimator().isStartFrameAnimator()) {
            if (!mChargeAnimator.getCircleAnimator().isStartAnimator()) {
                // mChargeAnimator.setVisibility(View.VISIBLE);
                mChargeAnimator.particleEffect();
                mChargeAnimator.getCircleAnimator().startScreenON();
            }
        }
    }
    //add by wangying end
    // add by tj start
    public void setPluggedAnimation(){
		//add by wangying begin20170309
        if(mChargeAnimator != null && !mChargeAnimator.isStartAnim()
                && StatusBarState.KEYGUARD == getBarState()){
            mChargeAnimator.setChargeState(mChargeAnimator.CHARGE_PULL_IN);
        }

//        mKeyguardIndicationController.setVisible(false);
//        chargeStateTv.setAlpha(0.0f);
//        chargeStateTv.setVisibility(View.VISIBLE);
//
//        keyguardCover.setAlpha(0.0f);
//        keyguardCover.setVisibility(View.VISIBLE);
//
//        AnimatorSet animatorSet = new AnimatorSet();
//        ObjectAnimator anim1 = ObjectAnimator.ofFloat(btPanelBottom, "alpha", 1.0f,0.0f);
//        anim1.setDuration(150);
//        ObjectAnimator anim2 = ObjectAnimator.ofFloat(chargeStateTv, "alpha", 0.0f,1.0f);
//        anim2.setDuration(150);
//        ObjectAnimator anim3 = ObjectAnimator.ofFloat(keyguardCover, "alpha", 0.0f,0.08f);
//        anim3.setDuration(400);
//        ObjectAnimator anim4 = ObjectAnimator.ofFloat(keyguardCover, "alpha", 0.08f,0.08f);
//        anim4.setDuration(200);
//        ObjectAnimator anim5 = ObjectAnimator.ofFloat(keyguardCover, "alpha", 0.08f,0.0f);
//        anim5.setDuration(400);
//        ObjectAnimator anim6 = ObjectAnimator.ofFloat(keyguardCover, "alpha", 0.0f,0.08f);
//        anim6.setDuration(400);
//        ObjectAnimator anim7 = ObjectAnimator.ofFloat(keyguardCover, "alpha", 0.08f,0.08f);
//        anim7.setDuration(200);
//        ObjectAnimator anim8 = ObjectAnimator.ofFloat(keyguardCover, "alpha", 0.08f,0.0f);
//        anim8.setDuration(400);
//        ObjectAnimator anim9 = ObjectAnimator.ofFloat(chargeStateTv, "alpha", 1.0f,0.0f);
//        anim7.setDuration(0);
//        ObjectAnimator anim10 = ObjectAnimator.ofFloat(btPanelBottom, "alpha", 0.0f,1.0f);
//        anim8.setDuration(0);
//
//        animatorSet.playSequentially(anim1,anim2,anim3,anim4,anim5,anim6,anim7,anim8,anim9,anim10);
...
//                mKeyguardIndicationController.setVisible(true);
//                chargeStateTv.setVisibility(View.GONE);
//            }
//
//            @Override
...
//        });
//        animatorSet.start();
		//add by wangying end 20170309
    }
    // add by tj end

    //add by linwujia begin
    public ImageView getBackDropBack() {
        return mBackdropBack;
    }

    public void setReturnCall(boolean showing) {
        returnCallShowing = showing;
    }

    public boolean returnCallShowing() {
        return returnCallShowing;
    }

    public WallpaperManager getWallpagerManager() {
        return mWallpaperManager;
    }
    //add by linwujia end

    //add begin by lych 20170315
    public boolean isPowerPluggedIn(){
        return mKeyguardIndicationController!=null?mKeyguardIndicationController.isPowerPluggedIn():false;
    }
    // add end by lych 20170315

    // add begin by lych for fix bug ncd#10215
    public int getQsMaxExpansionHeight(){
        return mNotificationPanel.mQsMaxExpansionHeight;
    }
    public int getQsMinExpansionHeight(){
        return mNotificationPanel.mQsMinExpansionHeight;
    }
    // add end by lych for fix bug ncd#10215
}
