...
public class NotificationPanelView extends PanelView implements
    ...
    public void setStatusBar(PhoneStatusBar bar) {
        mStatusBar = bar;
        mNotificationContainerParent.setStatusBar(bar);
    }
    ...
    protected void onFinishInflate() {
        ...
        mTelephoneBackViewTouchHelper = new TelephoneBackViewTouchHelper(mContext);
        //linwujia add end
        ...
        mKeyguardBottomArea.setPreviewContainer((ViewGroup) findViewById(R.id.preview_container));
        ...
        mQsAutoReinflateContainer.addInflateListener(new InflateListener() {
            ...
            public void onInflated(View v) {
                ...
                // talpa@andy 2017/3/27 14:23 delete @{
                /*   mQsContainer.getHeader().findViewById(R.id.expand_indicator)
                        .setOnClickListener(NotificationPanelView.this);*/
                // @}
                //talpa zhw add
                mQsContainer.findViewById(R.id.arrow_img).setOnClickListener(NotificationPanelView.this);
                mQsContainer.getHeader().findViewById(R.id.arrow_img).setOnClickListener(NotificationPanelView.this);
                //talpa zhw add end
                ...
                mNotificationContainerParent.setQsContainer(mQsContainer); // add by lych
    ...
    public void setClearAllViewHide(){
        mNotificationContainerParent.setClearAllViewHide();
    }
    // add end by lych
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if ((!mIsExpanding || mHintAnimationRunning)
            ...
            /*mStatusBar.getBarState() != StatusBarState.SHADE*/) {// add by wangying
...
//        Log.i(TAG, "shouldQuickSettingsIntercept="+shouldQuickSettingsIntercept(event.getX(), event.getY(), -1));
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN
    ...
    private void onQsTouch(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mStatusBar.isSuperPowerSaveMode()){
                    break;
                }
                // @}
                setQsExpansion(h + mInitialHeightOnTouch);
    ...
    private void setQsExpansion(float height) {
        ...
        if (height > mQsMinExpansionHeight && !mQsExpanded && !mStackScrollerOverscrolling) {
            ...
            setQsExpanded(false);
            mLastOverscroll = 0; /// George:modified for when tab back key, notificationStackScoller springback
    ...
    protected void updateQsExpansion() {
        ...
        mNotificationContainerParent.setExpandFraction(getQsExpansionFraction(),mExpandedHeight); // add by lych
    }
    ...
    private float calculateQsTopPadding() {
        ...
        //talpa zhw add
        }else if(mQsContainer.isCustomizing())
        {
    ...
    private void flingSettings(float vel, final boolean expand, final Runnable onFinishRunnable,
                               boolean isClick) {
        if(mStatusBar.isSuperPowerSaveMode()){
            return;
        }
        ...
        if (target == mQsExpansionHeight) {
            ...
            // Talpa:PeterHuang add for enforce to SHADE_LOCKED if have notifications in keyguard@{
            if (expand && mStatusBarState == StatusBarState.KEYGUARD
                    /*&& mStatusBar.hasActiveNotifications()*/){
                animateToFullShade(0 /* delay */);
                mStatusBar.setBarState(StatusBarState.SHADE_LOCKED);
                mStatusBar.updateKeyguardState(false /* goingToFullShade */, false /* fromShadeLocked */);
            }
            // @}
            return;
        }
    ...
    protected void onHeightUpdated(float expandedHeight) {
        ...
        //talpa zhw add
        boolean isCustomizer = mQsContainer.isCustomizing();
        //talpa zhw add end
        if (isCustomizer/*talpa zhw add*/ ||mQsExpandImmediate || mQsExpanded && !mQsTracking && mQsExpansionAnimator == null
                && !mQsExpansionFromOverscroll) {
            float t;
            if (mKeyguardShowing) {
    ...
    private int calculatePanelHeightQsExpanded() {
        float notificationHeight = mNotificationStackScroller.getHeight()
                ...
                - 0; //mNotificationStackScroller.getTopPadding(); // modified by lych for fix bug cdn#10023
    ...
    private void setListening(boolean listening) {
        ...
        keyguard_call_back.setListening(listening);
    }
    ...
    public void onQsHeightChanged() {
        mQsMaxExpansionHeight = mQsContainer.getDesiredHeight();
        if (mQsExpanded && mQsFullyExpanded
                ||mQsContainer.isCustomizing()/*talpa zhw add*/) {
            mQsExpansionHeight = mQsMaxExpansionHeight;
            requestScrollerTopPaddingUpdate(false /* animate */);
            requestPanelHeightUpdate();
        }
    ...
    public void onClick(View v) {
        if (/*v.getId() == R.id.expand_indicator|| */v.getId() == R.id.arrow_img) {//talpa zhw add
            // talpa@andy 2017/6/7 19:16 modify @{
            onQsExpansionOrCollapse();
            // @}
        }
    }

    // talpa@andy 2017/6/7 18:21 add @{
    /**
     * 快捷面板展开或者收缩
     */
    public void onQsExpansionOrCollapse() {
        onQsExpansionStarted();
        if (mQsExpanded) {
   ...
   // @}
            ...
            if (mFalsingManager.shouldEnforceBouncer()) {
                mStatusBar.executeRunnableDismissingKeyguard(new Runnable() {
                    ...
                    public void run() {
                        ...
                        //add by wangying 9065 小语种模式下,锁屏页上拉相机图标，手机无响应
    ...
    public void onIconClicked(boolean rightIcon) {
        ...
        //add by wangying begin fix cdn 9061
...
//        }
    ...
    public KeyguardAffordanceView getLeftIcon() {
        ...
        return mKeyguardBottomArea.getRightView();
    }
    ...
    public View getLeftPreview() {
        ...
        return mKeyguardBottomArea.getRightPreview();
    }
