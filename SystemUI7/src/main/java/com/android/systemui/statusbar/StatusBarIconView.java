...
public class StatusBarIconView extends AnimatedImageView {
    ...
    private void updateIconScale() {
        ...
        if(mNotification != null) {
            updateNotificationIconScale();
        } else {
            Resources res = mContext.getResources();
            final int outerBounds = res.getDimensionPixelSize(R.dimen.status_bar_icon_size);
            final int imageBounds = res.getDimensionPixelSize(R.dimen.status_bar_icon_drawing_size);
            final float scale = (float) imageBounds / (float) outerBounds;
            Log.d("StatusBar", "scale:" + scale);
            setScaleX(scale);
            setScaleY(scale);
        }
    ...
    //linwujia add begin /// George:changed for image scale
    public void updateNotificationIconScale() {
        Resources res = mContext.getResources();
        final int outerBounds = res.getDimensionPixelSize(R.dimen.itel_status_bar_icon_size);
        final int imageBounds = res.getDimensionPixelSize(R.dimen.itel_status_bar_icon_drawing_size);
        final float scale = (float)imageBounds / (float)outerBounds;
        Log.d("StatusBar", "scale:" + scale);
        setScaleX(scale);
        setScaleY(scale);
     ...
     */
    public boolean set(StatusBarIcon icon, String packageName) {
        ...
        if (!iconEquals) {
            if (!updateDrawable(false /* no clear */, mPackageName)) return false;
        }
        if (!levelEquals) {
    ...
    public void updateDrawable() {
        updateDrawable(true /* with clear */, mPackageName);
    }

    private boolean updateDrawable(boolean withClear, String packageName) {
        if (mIcon == null) {
            return false;
        }
        Drawable drawable = getIcon(mIcon, packageName);
        if (drawable == null) {
    ...
    private Drawable getIcon(StatusBarIcon icon, String packageName) {
        return getIcon(getContext(), icon, packageName);
    }
     ...
     * @param packageName packageName to use to distinguish the notification which is need to redraw added by wujia.lin
     * @return Drawable for this item, or null if the package or item could not
     *         be found
     */
    public static Drawable getIcon(Context context, StatusBarIcon statusBarIcon, String packageName) {
        int userId = statusBarIcon.user.getIdentifier();
        if (userId == UserHandle.USER_ALL) {
            userId = UserHandle.USER_SYSTEM;
        }
        Drawable icon = null;
        ItelNotificationIconUtil itelNotificationIconUtil = ItelNotificationIconUtil.getInstance(context);
        if(itelNotificationIconUtil.itelNotificationIconEquals(packageName)) {
            icon = itelNotificationIconUtil.getDrawableByPackageName(packageName);
        } else {
