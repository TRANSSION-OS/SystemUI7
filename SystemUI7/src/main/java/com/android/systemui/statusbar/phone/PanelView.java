...
import com.android.systemui.telephone.TelephoneBackViewTouchHelper;
...
public abstract class PanelView extends FrameLayout {
    ...
    protected TelephoneBackView keyguard_call_back;
    ...
    private float iniTransY;
    private boolean isCollapseAll = false;

    protected TelephoneBackViewTouchHelper mTelephoneBackViewTouchHelper;
    //linwujia add end
    ...
    protected final void notifyExpandingFinished() {
        ...
        if (mExpanding) {
            mExpanding = false;
            onExpandingFinished();
            //talpa@tj 20170615 去除横幅通知在通知栏悬挂效果 @{
            if(mStatusBar.isFullyExpanded()){
    ...
    public boolean onTouchEvent(MotionEvent event) {
        // Talpa:PeterHuang add 省电模式不能拖动和点击面板 @{
        if (mStatusBar.getBarState() == StatusBarState.SHADE
                && mStatusBar.isSuperPowerSaveMode()) {
            // 返回通话还要生效

            if (event.getActionMasked() == MotionEvent.ACTION_UP) {
                if (mTelephoneBackViewTouchHelper.onMotionEventActionUp(event)) {
                    return true;
                }
            } else if(event.getActionMasked() == MotionEvent.ACTION_DOWN) {
                mTelephoneBackViewTouchHelper.onMotionEventActionDown(event);
            } else if(event.getActionMasked() == MotionEvent.ACTION_MOVE) {
                mTelephoneBackViewTouchHelper.onMotionEventActionMove(event);
            }
            return false;
        }
        //@}

        // Talpa:PeterHuang add Fixed bug cdn#9924 @{
        if (TalpaUtils.isMTKPlatform()) {
            if (mStatusBar.getBarState() == StatusBarState.SHADE
                    && PowerOffAlarmManager.isAlarmBoot()) {
                LogUtil.d(" is isAlarmBoot here ignore");
                return false;
            }
        }
        // @}
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mJustPeeked && (!mGestureWaitForTouchSlop || mTracking) && !isTrackingBlocked()) {
                    ...
                    if (/*!mStatusBar.mStatusBarWindow.isLaunchCamera && */Math.abs(h) > mTouchSlop
                            && (Math.abs(h) > Math.abs(x - mInitialTouchX))) {
                        Log.v("PanelView", "onTouchEvent PanelView2222222:" + h + ":" + newHeight + ":" + mExpandedHeight);
                        setKeyguardBottom((int) (h));
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                LogUtil.d("linwujia ACTION_UP");
                //linwujia edit begin@{
                // shaolin add start
                /*int statusHeight = getResources().getDimensionPixelSize(R.dimen.status_bar_height);
                if(keyguard_call_back.getVisibility() == View.VISIBLE) {
                    if(y < statusHeight) { // Fix CDN 6502.插拔USB不方便调试，暂时这么改
                        resumeCall();
                    }
                }
                if(isTouchOnReturnCall(y)) {
                    return true;
                }*/
                if(mStatusBar.returnCallShowing() && mTelephoneBackViewTouchHelper.onMotionEventActionUp(event)) {
                    if(mStatusBar.getBarState() == StatusBarState.KEYGUARD)
                        return true; // 防止屏幕抖动
                }
                //}linwujia edit end
    ...
    private void endMotionEvent(MotionEvent event, float x, float y, boolean forceCancel) {
        ...
        if ((mTracking && mTouchSlopExceeded)
            ...
            //linwujia add begin
            float dY = Math.abs(y - mInitialTouchY);
            float dX = Math.abs(x - mInitialTouchX);
            //fling(vel, expand, isFalseTouch(x, y));
            //onTrackingStopped(expand);
            if(mStatusBar.getBarState() == StatusBarState.KEYGUARD && (dY < dX) && (dX > mTouchSlop)){
                fling(vel, true, isFalseTouch(x, y));
    ...
    protected void flingToHeight(float vel, boolean expand, float target,
        ...
        if (target == mExpandedHeight || getOverExpansionAmount() > 0f && expand) {
            ...
            //add by liuchen 20160519 解决锁屏滑动一半时中断操作，锁屏会停留在解锁一半的界面（概率50%）问题
            if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
                resetKeyguardBottom();
            }
            return;
        ...
        if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
            Log.v("PanelView", "onTouchEvent PanelView ACTION_UPyyyy:" + mExpandedFraction);
            if (!isCollapseAll) {
                resetKeyguardBottom();
            }
        }
        ValueAnimator animator = createHeightAnimator(true, target);
        if (expand) {
    ...
    protected void startUnlockHintAnimation() {
        ...
        // the expansion.
        if (mHeightAnimator != null || mTracking || mStatusBar.isSuperPowerSaveMode()) {
            return;
        }
        cancelPeek();
        notifyExpandingStarted();
    ...
    private void startUnlockHintAnimationPhase1(final Runnable onAnimationFinished) {
        ...
        ValueAnimator animator = createHeightAnimator(false, target);
    ...
    private void startUnlockHintAnimationPhase2(final Runnable onAnimationFinished) {
        ValueAnimator animator = createHeightAnimator(false, getMaxPanelHeight());
    ...
    public void setKeyguardBottomAnmi(boolean isFromFling, float iniHeight, float targetHeight) {
        try {
//            View vBack = mStatusBar.getBackDropBack();
            if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
            ...
            if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
                float dy = targetHeight - iniHeight;
                //add by wangying begin fix cdn#9734
                float translationY =  iniTransY + dy/2;
                if(translationY > 0){
                    translationY = 0;
                }
                //add by wangying end
                mKeyguardBottomArea.setTranslationY(translationY/*iniTransY + dy/2*/);
//                vBack.setTranslationY(translationY/*iniTransY+dy/2*/);
            }
        } catch (Exception e) {
    ...
    private ValueAnimator createHeightAnimator(final boolean isFromFling, final float targetHeight) {
        View vBack = mStatusBar.getBackDropBack();
        final float mIniHeight = mExpandedHeight;
        if (isFromFling && (targetHeight != 0)&&!mTouchAboveFalsingThreshold) {
            iniTransY = 0;
            mKeyguardBottomArea.setTranslationY(iniTransY);
            vBack.setTranslationY(iniTransY);
        } else {
            if(!isCollapseAll){
                iniTransY = mKeyguardBottomArea.getTranslationY();
            }
        }
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (!isFromFling||targetHeight==0) {
                    setKeyguardBottomAnmi(isFromFling, mIniHeight, (Float) animation.getAnimatedValue());
                }
            }
        });
        return animator;
    ...
    // ysg add begin
    public void setKeyguardBottom(int t) {
        try {

            //resetCamera();
            int transY = t;
            View v = mKeyguardBottomArea.getPanelViewBottom();
//            View back = mStatusBar.getBackDropBack();
            mKeyguardBottomArea.setBottomMaskVisibility(View.GONE);
            View indicationText = mKeyguardBottomArea.getIndicationView();
            if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
//                back.setVisibility(View.VISIBLE);
                ...
                mKeyguardBottomArea.setVisibility(View.INVISIBLE);
                //mKeyguardBottomBar.setVisibility(View.INVISIBLE);
            }
            ...
            if (mStatusBar.getBarState() == StatusBarState.KEYGUARD) {
                //int left = v.getLeft();
                int top = screenHeight + t - v.getHeight();// t- v.getHeight() ;//v.getTop() + dy;
                //int right = v.getRight();
                int bottom = screenHeight + t;// t;//v.getBottom() + dy;
                if (top < 0) {
                    top = 0;
                ...
                if(t==mMaxHeight){
//                    if(back.getTranslationY()!=0){
//                        back.setTranslationY(0);
//                    }
                    if(mKeyguardBottomArea.getTranslationY() != 0){
                        mKeyguardBottomArea.setTranslationY(0);
                    }

                }else{
                      iniTransY = bottom - mKeyguardBottomArea.getHeight();
//                    back.setTranslationY(transY/*bottom - back.getHeight()*/);
                    mKeyguardBottomArea.setTranslationY(transY);
                }
...
//                        back.setTranslationY(0);
//                    }
//                    if(indicationText.getTranslationY() != 0) {
//                        indicationText.setTranslationY(0);
//                    }
//                }else{
//                    iniTransY = bottom - back.getHeight();
//                    back.setTranslationY(bottom - back.getHeight());
//                    indicationText.setTranslationY(iniTransY);
//                }
            }
        } catch (Exception e) {
    ...
    public void resetKeyguardBottom() {
        if (mStatusBar.getBarState() == StatusBarState.KEYGUARD){
            Log.d("YSG6", "resetKeyguardBottom2222=");
//            View back = mStatusBar.getBackDropBack();
            iniTransY = 0;
            isCollapseAll = false;
            if (mKeyguardBottomArea.getTranslationY() != 0) {
...
//                back.setTranslationY(0);
//            }
            setKeyguardBottom((int) mMaxHeight);
            mKeyguardBottomArea.setBottomMaskVisibility(View.VISIBLE);
    ...
    //linwujia add begin
    /*private boolean isTouchOnReturnCall(float y) {
        if(getTelecommManager().isInCall() && mStatusBar.returnCallShowing()) {
            int statusHeight = getResources().getDimensionPixelSize(R.dimen.status_bar_height);
            if(y < statusHeight) { // 点击返回通话
                resumeCall();
                if(mStatusBar.getBarState() == StatusBarState.KEYGUARD)
                    return true; // 防止屏幕抖动
            }
