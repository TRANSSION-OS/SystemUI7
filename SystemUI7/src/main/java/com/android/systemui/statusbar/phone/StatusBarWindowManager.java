...
public class StatusBarWindowManager implements RemoteInputController.Callback {
    ...
    private final String CLEAR_RECENT_TASK = "android.intent.action.SUPER_CLEAR_RECENT_TASK";
    // @}
    ...
    public StatusBarWindowManager(Context context) {
        ...
        mScreenBrightnessDoze = mContext.getResources().getInteger(
                Resources.getSystem().getIdentifier("config_screenBrightnessDoze", "integer","android")) / 255f;

        // Talpa:PeterHuang add @{
        IntentFilter filter = new IntentFilter();
        filter.addAction(CLEAR_RECENT_TASK);
        mContext.registerReceiver(clearReceiver, filter);
        // @}
    }
    ...
    private BroadcastReceiver clearReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(CLEAR_RECENT_TASK)) {
                int isPowerSave = intent.getIntExtra("isPowerSave", 1); // 1 是超级省电   0 非超级省电
                if (isPowerSave == 1) {
                    LogUtil.d("PowerSave clear all task");
                    RecentsActivityUtils.getInstance().removeAllTasks(context);
                }
            }
        }
    };
...
//            mLpChanged.flags |= WindowManager.LayoutParams.FLAG_ALT_FOCUSABLE_IM;
    ...
    public void setStatusBarState(int state) {
        ...
        //linwujia add begin
        StatusBarWindowView mSBWV = ((StatusBarWindowView)mStatusBarView);
        if(getTelecommManager().isInCall() && state == StatusBarState.KEYGUARD){
            mSBWV.keyguardTelephoneBack(View.VISIBLE);
        } else {
            mSBWV.keyguardTelephoneBack(View.INVISIBLE);
        }
        //linwujia add end
    }
