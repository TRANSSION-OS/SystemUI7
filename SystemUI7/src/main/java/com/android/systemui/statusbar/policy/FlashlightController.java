    ...
    public void setFlashlight(boolean enabled) {
        ...
        synchronized (this) {
            if (mFlashlightEnabled != enabled) {
                ...
                if (!Settings.System.putInt(mContext.getContentResolver(), "flashlight_open", enabled ? 1 : 0))  {
                    LogUtil.e("write flashlight_open failed: " + enabled);
                }
                // @}
                try {
                    ...
                    // Talpa:PeterHuang add @{
                    if (!Settings.System.putInt(mContext.getContentResolver(), "flashlight_open", 0)){
                        LogUtil.e("catch write flashlight_open failed: " + enabled);
                    }
                    // @}
                    pendingError = true;
                }
    ...
    private final CameraManager.TorchCallback mTorchCallback =
        ...
        private void setCameraAvailable(boolean available) {
            ...
            synchronized (FlashlightController.this) {
                ...
                // Talpa:PeterHuang add  相机打开？给长按Power面板通信。  @{
                if(!available){
                    if (!Settings.System.putInt(mContext.getContentResolver(), "flashlight_open", 0)){
                        LogUtil.e("write flashlight_open failed" );
                    }
                }
                // @}
            }
