...
public class KeyguardAffordanceHelper {
    ...
    private View getIconAtPosition(float x, float y) {
        if (leftSwipePossible() && isOnIconWithRaw(mLeftIcon, x, y)) {
            return mLeftIcon;
        }
        if (rightSwipePossible() && isOnIconWithRaw(mRightIcon, x, y)) {
            return mRightIcon;
        }
        return null;
    }
    ...
    //linwujia add begin
    private boolean isOnIconWithRaw(View icon, float x, float y) {
        View firstParent = (View) icon.getParent();
        View secondParent = (View) icon.getParent().getParent();
        float firstParentX = firstParent.getX();
        float firstParentY = firstParent.getY();
        float secondParentX = secondParent.getX();
        float secondParentY = secondParent.getY();
        float iconX = firstParentX + secondParentX + icon.getX() + icon.getWidth() / 2.0f;
        float iconY = firstParentY + secondParentY + icon.getY() + icon.getHeight() / 2.0f;
        double distance = Math.hypot(x - iconX, y - iconY);
        return distance <= mTouchTargetSize / 2;
    }
    //linwujia add end
    ...
    private float getRadiusFromTranslation(float translation) {
        ...
        float radius = (translation - mTouchSlop)  * BACKGROUND_RADIUS_SCALE_FACTOR + mMinBackgroundRadius;
        if(radius > mHintGrowAmount) {
            radius = mHintGrowAmount;
        }
        return radius;
    }
