...
public class NotificationStackScrollLayout extends ViewGroup
    ...
    private final Paint mAlphaBgPaint = new Paint(); // add by lych 20170306
    ...
    public NotificationStackScrollLayout(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        myColor = mContext.getColor(R.color.itel_notification_bg);
        chargingColor = mContext.getColor(R.color.charging_animation_background);
        initView(context);
    ...
    protected void onDraw(Canvas canvas) {
        //modify begin by lych 20170306
        //canvas.drawRect(0, mCurrentBounds.top, getWidth(), mCurrentBounds.bottom, mBackgroundPaint);
        if(mPhoneStatusBar.getBarState()==StatusBarState.KEYGUARD
                && mPhoneStatusBar.isPowerPluggedIn()
                && mDrawBackgroundAsSrc && (!mFadingOut && !mParentFadingOut)){
            //lych: in case that not lock wallpaper and charging on keyguard
            canvas.drawRect(0, mCurrentBounds.top, getWidth(), mCurrentBounds.bottom, mChargingAlphaBgPaint);
        }else if(mPhoneStatusBar.getBarState()==StatusBarState.KEYGUARD || mPhoneStatusBar.isPanelFullyCollapsed()){
            //lych: in case that has lock wallpaper on keyguard or on notification popwindows
            canvas.drawRect(0, mCurrentBounds.top, getWidth(), mCurrentBounds.bottom, mAlphaBgPaint);
        }else{
            canvas.drawRect(0, mCurrentBounds.top, getWidth(), mCurrentBounds.bottom, mBackgroundPaint);
        }
        //modify end by lych 20170306
    ...
    private void updateBackgroundDimming() {
      ...
      //  int myColor = mContext.getColor(R.color.itel_notification_bg);
        // modify begin by lych 20170306
//        mBackgroundPaint.setColor(myColor);
        mBackgroundPaint.setColor(color);
        mAlphaBgPaint.setColor(myColor);
        mChargingAlphaBgPaint.setColor(0XA6000000);
        // modify end by lych 20170306
        invalidate();
    }
    ...
    private void initView(Context context) {
        ...
        mSidePaddings = context.getResources()
                .getDimensionPixelSize(R.dimen.itel_notification_side_padding);
    ...
    private PorterDuffXfermode mXorMode = new PorterDuffXfermode(PorterDuff.Mode.DST);// add by lych 20170313
    private PorterDuffXfermode mDstInMode = new PorterDuffXfermode(PorterDuff.Mode.DST_ATOP); // used in not lock wallpaper and charging
    private PorterDuffXfermode mDstAtopMode = new PorterDuffXfermode(PorterDuff.Mode.CLEAR); // used in not lock wallpaper and not charging
    private Paint mChargingAlphaBgPaint = new Paint(); //used in not lock wallpaper
    private void updateSrcDrawing() {
        mBackgroundPaint.setXfermode(mDrawBackgroundAsSrc && (!mFadingOut && !mParentFadingOut)
                ? mSrcMode : null);
        //add begin for the clear all button be removed from NotificationStackScrollLayout by lych 20170306
        mAlphaBgPaint.setXfermode(mDrawBackgroundAsSrc && (!mFadingOut && !mParentFadingOut)
                ? mDstAtopMode : mXorMode);
        mChargingAlphaBgPaint.setXfermode(mDrawBackgroundAsSrc && (!mFadingOut && !mParentFadingOut)
                ? mDstInMode : null);
        // add end for the clear all button be removed from NotificationStackScrollLayout by lych 20170306
        invalidate();
    }
...
//        }
    ...
    private void setOverScrollAmountInternal(float amount, boolean onTop, boolean animate,
        ...
        int distance = mPhoneStatusBar.getQsMaxExpansionHeight() - mPhoneStatusBar.getQsMinExpansionHeight();
        if(amount>distance){
            LogUtil.d(" lych: amount="+amount+", onTop="+onTop+", animate="+animate
                    +", isRubberbanded="+isRubberbanded+", disance="+distance);
            amount = distance;
        }
    ...
    public void updateTopPadding(float qsHeight, boolean animate,
        ...
        start += mSidePaddings; // add by lych for fix bug cdn#10023
...
//        expandableView.setClipTopAmount(0);
    ...
    public void setDismissView(DismissView dismissView) {
        ...
        //addView(mDismissView, index); // removed for the clear all button be removed from NotificationStackScrollLayout by lych 20170306
    }
    ...
    public void updateEmptyShadeView(boolean visible) {
        ...
        // Talpa:PeterHuang fixed tfs#15672 @{
        if (newVisibility == VISIBLE && mPhoneStatusBar.isSuperPowerSaveMode()) {
            //if (mPhoneStatusBar.getBarState() ==StatusBarState.KEYGUARD){
            LogUtil.d("not updateEmptyShadeView");
            newVisibility = GONE;
            //}
        }
        // @}
        if (oldVisibility != newVisibility) {
            if (newVisibility != GONE) {
                ...
                // add begin by lych for fix bug cdn#10195
                if(mPhoneStatusBar.getBarState()==StatusBarState.KEYGUARD && newVisibility==GONE){
                    mEmptyShadeView.setInvisible();
                }
                // add end by lych for fix bug cdn#10195
