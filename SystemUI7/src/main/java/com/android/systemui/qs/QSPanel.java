...
import itel.transsion.settingslib.utils.NoDoubleClickListener;
...
public class QSPanel extends LinearLayout implements Tunable, Callback {
    ...
    protected View mArrowGroup;
    private IQuickSettingsPlugin mQuickSettingsPlugin;
    // talpa@andy 2017/6/7 20:48 add @{
    ...
    public QSPanel(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
//        LogUtil.i("QSTileView:onConfigurationChanged");
        ...
        //@talpa andy delete @{
        //talpa zhw add
//        LayoutParams params = (LayoutParams) mBrightnessView.getLayoutParams();
        //@}
        //talpa@andy modify @{
//        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.BrightnessDialog);
//        float marginStart = ta.getDimension(R.styleable.BrightnessDialog_marginStart, 0);
//        float marginEnd = ta.getDimension(R.styleable.BrightnessDialog_marginEnd, 0);
//        int height = getResources().getDimensionPixelSize(R.dimen.qs_content_brightness_view_height);
//        LinearLayout.LayoutParams lyParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,  height);
//        lyParams.setMarginStart((int) marginStart);
//        lyParams.setMarginEnd((int) marginEnd);
        addView(mBrightnessView);
        //@}
        View v = LayoutInflater.from(context).inflate(R.layout.talpa_arrow_layout, this, false);
        mArrow = (ImageView) v.findViewById(R.id.arrow_img);
        addView(v);
        mArrowGroup = v;
        //talpa zhw add end
        ...
        mBrightnessController = new BrightnessController(getContext(),
                (ImageView) findViewById(R.id.brightness_icon),
                (ToggleSlider) findViewById(R.id.brightness_slider),
                (ImageView) findViewById(R.id.brightness_auto_control_icon));
        //talpa zhw add end
    }
    // talpa@andy 2017/6/7 20:48  @{
    public void setPanelView(NotificationPanelView mPanelView) {
    ...
    public void setArrowVisibility(int vis)
    {
        mArrowGroup.setVisibility(vis);
    }

    public void setBrightnessAlpha(float alpha)
    {
        if(alpha > 0.9) {
            mBrightnessView.setAlpha((alpha * 10 - 9) / 1f);
        }
        else
        {
            mBrightnessView.setAlpha(0);
        }
    }
    //talpa zhw add end
    ...
    //talpa zhw add
    public void handleeEditClickEvent(final View view)
    {
        mHost.startRunnableDismissingKeyguard(new Runnable() {
            @Override
            public void run() {
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        if (mBrightnessMirrorController != null) {
            ...
            // talpa@andy 2017/3/7 17:36 add: register callback @{
    ...
    protected QSTileBaseView createTileView(QSTile<?> tile, boolean collapsedView) {
        ...
        {*/
            ...
            iconview.setQSPanel(QSPanel.this);
            return new QSTileView(mContext, iconview, collapsedView);
       /* }
        //talpa zhw add end
    ...
    protected void addTile(final QSTile<?> tile, boolean collapsedView) {
        ...
        r.callback = callback;
        final View.OnClickListener click = new NoDoubleClickListener() {
            @Override
            protected void onNoDoubleClick(View v) {
                onTileClick(r.tile, r.tileView);
            }
        };
        ...
        // talpa@andy 2017/5/10 15:39 fix bug:cdn#9956 下拉唤出通知栏，连续点击蓝牙快捷开关，
        // 动画切换效果很慢@{
        r.tileView.setQSTile(r.tile);
        // @}
        r.tile.refreshState();
        mRecords.add(r);

        if (mTileLayout != null) {//viewpager
