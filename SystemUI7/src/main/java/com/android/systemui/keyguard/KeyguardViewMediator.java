...
public class KeyguardViewMediator extends SystemUI {
                ...
                case READY:
                    synchronized (this) {
                        if (mShowing) {
                            ...
                            boolean disable = !mLockPatternUtils.isLockScreenDisabled(currentUser);
                            if(disable){
                                resetStateLocked();
                            }
                        }
    ...
    private void setupLocked() {
        ...
        int lockSoundDefaultAttenuation = mContext.getResources().getInteger(
                Resources.getSystem().getIdentifier("config_lockSoundVolumeDb", "integer","android"));
        mLockSoundVolume = (float)Math.pow(10, (float)lockSoundDefaultAttenuation/20);

        mHideAnimation = AnimationUtils.loadAnimation(mContext,
                Resources.getSystem().getIdentifier("lock_screen_behind_enter", "anim","android"));
        /// George:add for AlarmBoot---2
        if(TalpaUtils.isMTKPlatform()) {
    ...
    private void doKeyguardLocked(Bundle options) {
        ...
        /// George:add for AlarmBoot---4
        if (!mExternallyEnabled || (TalpaUtils.isMTKPlatform()&&PowerOffAlarmManager.isAlarmBoot())) {
            if (DEBUG) {
    ...
    public void handleDismiss() {
        if (DEBUG) {
            Log.d(TAG, "handleDismiss; mShowing = " + mShowing + "; mOccluded = " + mOccluded);
        }
    ...
    private void handleKeyguardDone(boolean authenticated) {
        ...
        ///M: [ALPS00827994] always to play sound for user to unlock keyguard /// George: change it for BootAlarm
    ...
    private void playSounds(boolean locked) {
        // User feedback for keyguard./// George: add for AlarmBoot
