package itel.transsion.systemui.ChargingLeonids.leonids;
...
import itel.transsion.systemui.ChargingLeonids.leonids.initializers.AccelerationInitializer;
import itel.transsion.systemui.ChargingLeonids.leonids.initializers.ParticleInitializer;
import itel.transsion.systemui.ChargingLeonids.leonids.initializers.RotationInitiazer;
import itel.transsion.systemui.ChargingLeonids.leonids.initializers.RotationSpeedInitializer;
import itel.transsion.systemui.ChargingLeonids.leonids.initializers.ScaleInitializer;
import itel.transsion.systemui.ChargingLeonids.leonids.initializers.SpeeddByComponentsInitializer;
import itel.transsion.systemui.ChargingLeonids.leonids.initializers.SpeeddModuleAndRangeInitializer;
import itel.transsion.systemui.ChargingLeonids.leonids.modifiers.AlphaModifier;
import itel.transsion.systemui.ChargingLeonids.leonids.modifiers.ParticleModifier;


public class ParticleSystem {

	private static final long TIMMERTASK_INTERVAL = 80;
	...
	public ParticleSystem(ViewGroup parentView, int maxParticles, Drawable drawable, long timeToLive) {
		...
		if (drawable instanceof AnimationDrawable) {
			...
			for (int i=0; i<mMaxParticles; i++) {
				mParticles.add (new Particle (bitmap, mParentView.getContext()));
			}
		}
	}
	...
	private void startEmiting(int particlesPerSecond) {
		...
		int width = mParentView.getResources().getDimensionPixelOffset(R.dimen.keyguard_charging_particle_width);
		int height = mParentView.getResources().getDimensionPixelOffset(R.dimen.keyguard_charging_particle_height);
		mDrawingView.layout(0, 0, width/*mParentView.getWidth()*/, height/*mParentView.getHeight()*/);
